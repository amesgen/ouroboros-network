...
-- DUPLICATE -- adapted from: cardano-api/src/Cardano/Api/KeysByron.hs
...
instance SerialiseAsRawBytes (SigningKey ByronKeyLegacy) where
    ...
    deserialiseFromRawBytes (AsSigningKey AsByronKeyLegacy) bs =
     ...
     where
      ...
      decodeXPrv :: CBOR.Decoder s Crypto.HD.XPrv
      decodeXPrv = CBOR.decodeBytesCanonical >>= toCborError . Crypto.HD.xprv
