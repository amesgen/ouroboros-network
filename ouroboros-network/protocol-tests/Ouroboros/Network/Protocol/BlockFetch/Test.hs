...
genBlockFetch :: Gen block
              ...
              -> Gen (AnyMessage (BlockFetch block point))
genBlockFetch genBlock genChainRange = oneof
    [ AnyMessage . MsgRequestRange <$> genChainRange
    , return $ AnyMessage MsgStartBatch
    , return $ AnyMessage MsgNoBlocks
    , AnyMessage . MsgBlock <$> genBlock
    , return $ AnyMessage MsgBatchDone
    , return $ AnyMessage MsgClientDone
    ]

instance Arbitrary (AnyMessage (BlockFetch Block (Point Block))) where
...
instance Arbitrary (AnyMessage (BlockFetch (Serialised Block) (Point Block))) where
